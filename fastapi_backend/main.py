...
from fastapi_backend.database import engine, SessionLocal  # ✅ Corrected import
...
def update_dashboard(
    ...
    name: str = Form(...),
    password: str = Form(...),
    course_interest: List[str] = Form(...),  # <--- handles multiple values
    db: Session = Depends(get_db)
):
    user = db.query(models.User).filter(models.User.id == user_id).first()
    user.name = name
    user.password = password
    user.course_interest = ",".join(course_interest)  # save as CSV string
    db.commit()
    return RedirectResponse(url=f"/dashboard/{user_id}", status_code=302)

# ------------------------------
# NEW: Update user_id (if needed)
